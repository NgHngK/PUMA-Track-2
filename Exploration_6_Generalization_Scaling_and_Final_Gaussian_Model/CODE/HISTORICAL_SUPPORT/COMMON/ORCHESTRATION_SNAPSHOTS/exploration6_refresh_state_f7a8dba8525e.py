...
Stage B raw AUG0 seed17 is active. A finite runner waits for this existing process then executes the remaining eight already frozen configs sequentially; status is RESEARCH_STATE/AUGMENTATION_PIPELINE_STATUS.json. Stage C cached no-augmentation sampling/cRT controls completed under frozen SCHEDULING_AMENDMENT_01 and remain conditional until B resolves. Mean ROI F1: inverse0.149770, natural0.138005, tempered0.140028, cRT reset0.137769, cRT no-reset0.140762. All fail promotion. Reset lowers gap0.478312 to0.390362 but also lowers ROI F1; gap reduction alone is insufficient. Exact values: TABLES/CACHED_LONG_TAIL_SUMMARY.json. Never restart existing partial or completed run directories. Read run_registry and raw logs for current execution state.

Current completed-trial ledgers cover240 epochs, including30 decoupled epochs, with verified sampler replay. The standalone A5 package preserves21 completed runs. All80 test cases passed across the initial79 passes and the corrected missing-vendor parity fixture test. Historical original/archive/master recheck passed17,617 checks; initial421 missing-path reports were Windows long-path false negatives, not lost files. Exploration5 has96 verified archival copies. The additive historical supplement contains473 canonical histories and all18 unresolved imported epochs; it has not been appended to the master. Figures are under FIGURES with input hashes and source scripts.
