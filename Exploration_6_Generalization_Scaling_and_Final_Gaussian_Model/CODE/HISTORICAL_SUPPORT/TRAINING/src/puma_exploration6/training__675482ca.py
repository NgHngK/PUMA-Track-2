...
def _forward(model,batch,device,encoder=None):
    ...
    if "tokens" in batch:return model(batch["tokens"].to(device,non_blocking=True),bio,batch["u"].to(device),batch["v"].to(device))
    ...
    return model(tokens,bio,batch["u"].to(device),batch["v"].to(device))
