...
def _make_loader(ds,labels,cfg: ExperimentConfig,train: bool,seed: int,sampler_cfg=None,batch_size: int | None=None):
    ...
    loader_kwargs=dict(
        batch_size=batch_size or cfg.optimizer.batch_size,shuffle=train and sampler is None,sampler=sampler,
        num_workers=cfg.num_workers,pin_memory=cfg.device.startswith("cuda") and torch.cuda.is_available(),
...
def run_experiment(cfg: ExperimentConfig) -> dict:
    ...
    criterion=PriorAdjustedCrossEntropy(q,cfg.loss).to(device)
    ...
    if cfg.decoupled.enabled:
        ...
        dcrit=PriorAdjustedCrossEntropy(dq,cfg.decoupled.loss).to(device)
        decay=[];no_decay=[]
