...
EXTERNAL_HOLDOUT_SPLITS = {"dev", "calibration", "locked", "locked_natural", "test"}
...
def create_internal_group_cv_manifests(
    ...
    *,
    ...
    # sklearn 1.7.2 shuffles count rows without remapping groups_inv. Randomize
    # group IDs instead, keeping each count vector attached to its real samples.
    # Stable sorting in the unshuffled splitter then breaks ties in seeded order.
    _, group_inverse = np.unique(train_groups, return_inverse=True)
    shuffled_group_ids = np.random.RandomState(seed).permutation(len(unique_groups))[group_inverse]
    ...
    for fold, (subtrain_pos, subdev_pos) in enumerate(splitter.split(X, y, groups=shuffled_group_ids)):
        ...
        metadata = {
            ...
            "n_splits": n_splits,
            "seed": seed,
            "splitter": "StratifiedGroupKFold(shuffle=False), seeded group-ID permutation",
