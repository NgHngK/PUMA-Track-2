# Exploration2 / Frozen-encoder control family FROZEN_UNI2_shuffle
...
Exploration2 isolates FOV, prior correction, biology and capacity after exact localV17parity.

## Exact forward

`z=Wh+a (+ Vb when biology is present), or z=Vb+a for biology-only`. sourcecrop→frozenUNI2CLS1536→nonaffineLN→linear10; optionalzero-initbiaslessbiologylinear. BIOLOGY_ONLY omits the encoder and consumes only its configured biology columns. The fixed parameter count for appearance-only is15,370; for fullTierA linear correction15,530; biology-onlyfull16is170. Ablation/subfamilycounts depend on exact selectedcolumns. Oracle dimensionality is recorded in its schema/normalizer, not inferred from thename.
