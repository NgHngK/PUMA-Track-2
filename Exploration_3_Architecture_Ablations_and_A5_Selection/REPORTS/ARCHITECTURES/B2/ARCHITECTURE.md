# Exploration3 / TierB stain/texture diagnostic
...
Measure mask-weighted stain contribution; contaminated upstream.

## Exact forward

`z=Wh+a+V b_stain`. CLS1536 plus6mask-weighted stain/texture features→10zero-init. Here h is nonaffine-LN1536, b is the selected feature vector standardized using only current training rows.

## Parameter count

15,430 trainable head parameters. Encoder frozen; no LoRA. Models and per-run measured counts are in RUNS/<ID>/summary.json.
