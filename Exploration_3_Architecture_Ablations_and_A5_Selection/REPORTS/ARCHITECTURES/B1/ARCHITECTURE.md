# Exploration3 / TierB morphology diagnostic
...
Measure morphology contribution; contaminated upstream.

## Exact forward

`z=Wh+a+V b_morph`. CLS1536 plus8mask morphology features→10zero-init. Here h is nonaffine-LN1536, b is the selected feature vector standardized using only current training rows.

## Parameter count

15,450 trainable head parameters. Encoder frozen; no LoRA. Models and per-run measured counts are in RUNS/<ID>/summary.json.
