# Exploration3 / Rank8 appearance×TierA interaction
...
Test cross-feature interactions with low parameter count.

## Exact forward

`z=Wh+a+Wr[(Ph h)⊙(Pb b)]`. 1536→8 and16→8, product8→10zero-init. Here h is nonaffine-LN1536, b is the selected feature vector standardized using only current training rows.

## Parameter count

27,866 trainable head parameters. Encoder frozen; no LoRA. Models and per-run measured counts are in RUNS/<ID>/summary.json.

Per component: appearance1536×10+10=15,370; Ph1536×8=12,288; Pb16×8=128; Wr8×10=80. Total27,866. InitialWr=0 gives exact appearance baseline; first-step gradient toPh/Pb is0, whileWr can receive gradient; afterWrupdates projections can learn. This intended delayed gradient is not minority starvation.
