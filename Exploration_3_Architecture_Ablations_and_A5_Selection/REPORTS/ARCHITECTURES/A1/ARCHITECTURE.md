# Exploration3 / Linear TierA correction
...
Test marginal value of direct RGB/point features.

## Exact forward

`z=Wh+a+Vb`. CLS1536 + standardizedRGB16 → independentLinear16→10 → sum logits. Here h is nonaffine-LN1536, b is the selected feature vector standardized using only current training rows.

## Parameter count

15,530 trainable head parameters. Encoder frozen; no LoRA. Models and per-run measured counts are in RUNS/<ID>/summary.json.
