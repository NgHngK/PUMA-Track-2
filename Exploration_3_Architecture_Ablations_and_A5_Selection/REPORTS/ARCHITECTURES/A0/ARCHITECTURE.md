...
Remove biology to establish appearance baseline.

## Exact forward

`z=Wh+a`. Frozen CLS1536 → nonaffine LN → Linear1536→10. Here h is nonaffine-LN1536, b is the selected feature vector standardized using only current training rows.

## Parameter count

15,370 trainable head parameters. Encoder frozen; no LoRA. Models and per-run measured counts are in RUNS/<ID>/summary.json.
