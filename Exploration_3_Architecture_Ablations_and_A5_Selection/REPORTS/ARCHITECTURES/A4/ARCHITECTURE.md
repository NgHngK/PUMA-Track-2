# Exploration3 / Uncertainty-gated TierA correction
...
Test whether biology is conditionally useful when appearance is uncertain.

## Exact forward

`z=Wh+a+sigmoid(w_g·[H(softmax(z_app))/log10,top1−top2]+c_g)Vb`. Detached appearance entropy/margin2→1sigmoid gate. Here h is nonaffine-LN1536, b is the selected feature vector standardized using only current training rows.

## Parameter count

15,533 trainable head parameters. Encoder frozen; no LoRA. Models and per-run measured counts are in RUNS/<ID>/summary.json.

The uncertainty inputs are detached, preventing gate-learning from manipulating appearance entropy directly. Gateweights/bias initialized0. Entropy normalized bylog10.
