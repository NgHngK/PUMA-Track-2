# Exploration3 / Concatenation bottleneck
...
Test low-dimensional joint bottleneck; cannot share linear appearance initialization.

## Exact forward

`z=W2 GELU(W1[h;b]+c1)+c2`. concat1552→32GELU→10; replaces appearance-linear head. Here h is nonaffine-LN1536, b is the selected feature vector standardized using only current training rows.

## Parameter count

50,026 trainable head parameters. Encoder frozen; no LoRA. Models and per-run measured counts are in RUNS/<ID>/summary.json.
