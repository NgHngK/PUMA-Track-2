# Exploration3 / TierA+TierB diagnostic
...
Test complementarity; combined linear36is algebraically separate16+20 corrections.

## Exact forward

`z=Wh+a+V_A b_A+V_B b_B`. CLS1536 plus36combined biology features→10zero-init. Here h is nonaffine-LN1536, b is the selected feature vector standardized using only current training rows.

## Parameter count

15,730 trainable head parameters. Encoder frozen; no LoRA. Models and per-run measured counts are in RUNS/<ID>/summary.json.
