# Exploration3 / Small nonlinear TierA correction
...
Test nonlinear biology without large fusion capacity.

## Exact forward

`z=Wh+a+V2 GELU(V1 b+c1)+c2`. 16→16GELU→10, final weights/bias zero. Here h is nonaffine-LN1536, b is the selected feature vector standardized using only current training rows.

## Parameter count

15,812 trainable head parameters. Encoder frozen; no LoRA. Models and per-run measured counts are in RUNS/<ID>/summary.json.

V1bias included272params;V2bias included170params. ZeroV2/c2 initially suppresses biology; V1learns only afteroutput moves.
