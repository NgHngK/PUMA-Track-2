...
D=R/'EXPLORATION_4/ARCHITECTURES/F_CLEAN_MASK_NETWORK';p=D/'CODE/run_eval.py';cp(p,D/'PROVENANCE/run_eval_archive_reader_preserved.txt')
p.write_text('''import argparse,json,sys
...
''',encoding='utf8')
readme=D/'README.md';cp(readme,D/'PROVENANCE/README_initial_draft.md');readme.write_text(readme.read_text().replace('`run_eval.py --complete RESULTS/RUNS/CLEAN_GT_ONLY/complete.json` reads archived endpoint metrics; it is not an independent checkpoint-forward test.','`run_eval.py --checkpoint RESULTS/RUNS/CLEAN_GT_ONLY/final.pt --output evaluation.json` strictly loads the checkpoint and evaluates all156 excluded fold0 nuclei. Optional `--reference-masks` checks forward probabilities against the archived mask cache.'),encoding='utf8')
print('Mask evaluation now performs checkpoint inference')
