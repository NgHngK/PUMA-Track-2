...
import os
A=P/'SHARED_INPUT_SNAPSHOTS/CLEAN_MASK_INPUTS';D=P/'BIOMASK_GUIDED';O=Path(os.environ.get('PUMA_RUN_OUTPUT','NEW_CLEAN_MASK'))
def load(n,p):
...
def prepare():
 rows,cv,y=load_data();rm=np.load(A/'roi_manifest.npy');folds=np.load(A/'folds.npy');mp=dict(zip(rm['roi_id'],folds));f=np.array([mp[r['roi']] for r in rows]);ix=np.flatnonzero(f!=0);rgb=np.load(A/'rgb_crops_qc.npy');gt=np.load(A/'gt_masks_diagnostic.npy');crop=load('clean_crop',Path(__file__).parent/'mask_source/crops.py');prompts=[];valid=[];trans=[];sizes={}
 for r in rows:
...
def main():
 torch.set_num_threads(4);torch.manual_seed(17);O.mkdir(parents=True,exist_ok=True)
 ...
 rows,tr,va,x,g,valid,trans=prepare();bm=load('clean_mask_model',Path(__file__).parent/'mask_source/biomask.py');m=bm.ProposalBiologyNetwork(32)
