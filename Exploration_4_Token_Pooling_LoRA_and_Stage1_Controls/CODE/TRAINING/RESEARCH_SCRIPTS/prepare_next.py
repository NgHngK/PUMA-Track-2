...
cp(W/'research-state.json',W/f'state_history/{now}.json')
s=json.loads((W/'research-state.json').read_text())
s.update(phase='A3 selected in internal CV; B, C alignment and D rejected; E LoRA running; archive verification ongoing',completed_runs=sorted(p.parent.name for p in (P/'RUNS').glob('*/summary.json')),sessions={'LoRA':42783,'package_verification':30303},next=['finish restricted LoRA three seeds','clean GT-only BioMask and finite pooling controls','final candidate confirmation','complete standalone packages and integrated master report','verify original hashes and archive lineage'])
dump(W/'research-state.json',s)
with (W/'findings.md').open('a',encoding='utf8') as f:
 f.write('\nD ROI-class sampling delta -0.000063; rejected. C clean detector-held matched cohort delta +0.005635, ROI interval [-0.006667,0.017857], apoptosis recall delta -0.166667; rejected. Historical 21 packages passed evaluator fixtures and execution checks. E restricted LoRA seed17 finished five epochs (ROI F1 0.1406453); seeds29/43 remain active, no decision yet. P4 packages copied; verification running.\n')
text='''# F implementation clarification before any mask or pooling fit
...
'''
dest=P/'PROVENANCE/F_IMPLEMENTATION_PROTOCOL.md'
if dest.exists():assert dest.read_text(encoding='utf8')==text
else:dest.write_text(text,encoding='utf8')
