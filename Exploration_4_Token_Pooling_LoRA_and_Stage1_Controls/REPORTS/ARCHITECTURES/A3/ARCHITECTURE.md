...
Exploration4 architecture ID: `A3`. **27,866 trainable parameters** for the executed configuration. Nine neighboring patch tokens; edges clipped. Passed internal CV but failed final class-recall confirmation.

## Mathematical and tensor contract

`h = LN(mean{T[9+j]: |j_x-floor(u)|≤1 and |j_y-floor(v)|≤1})`
