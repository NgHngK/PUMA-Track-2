# Clean GT-only mask network

Exploration4 architecture ID: `F_CLEAN_MASK_NETWORK`. **842,337 trainable parameters** for the executed configuration. Random initialization;294 non-fold0 GT-prompt training rows,156 held fold0; no semantic-label or Stage1-proposal training input.

## Mathematical and tensor contract

`L = .6 valid-pixel BCE + .4 (1-softDice)`
