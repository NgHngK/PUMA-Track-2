# CLS and clean mask mean

Exploration4 architecture ID: `F_CLS_MASK`. **27,866 trainable parameters** for the executed configuration. Equal mean, same head capacity; failed promotion.

## Mathematical and tensor contract

`h = LN((LN(CLS)+LN(mask pooled tokens))/2)`
